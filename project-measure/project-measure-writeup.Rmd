...
measure.df = data.frame(measure, stringsAsFactors = FALSE)
  
for (j in 1:length(measure.df$my.gender))
{
    if (measure.df[j,30] == "f")
    {
      measure.df[j,30] = "1"
    }
    if (measure.df[j,30] == "m")
    {
      measure.df[j,30] = "2"
    }
    if (measure.df[j,30] == "o")
    {
      measure.df[j,5] = "3"
    }
}
